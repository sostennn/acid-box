...
Ce document planifie la **v1** en détail et esquisse v2–v4. Aucune
implémentation n'est écrite à ce stade ; seuls les types TypeScript du modèle
de données apparaissent en code.
...
| **Vitest** (node) + **@testing-library/svelte** (jsdom) + **Vitest browser mode** (Playwright, Chromium) pour quelques tests audio réels sur `OfflineAudioContext` | Voir §6. La logique du moteur est testée sans Web Audio ; le mode navigateur ne sert qu'aux tests de rendu sonore.                                                                                                                                                                                                                                                                  |
...
│   │       └── params.ts           # helpers d'automation sûrs (min > 0, clamp temps)
│   │
...
│   └── main.ts
├── tests/
│   ├── browser/                    # tests OfflineAudioContext (vitest browser mode)
│   └── fakes/                      # FakeAudioContext, FakeClock, FakeTimer
├── eslint.config.js, .stylelintrc, vite.config.ts, vitest.config.ts, tsconfig.json
└── README.md
```
...
export interface EngineOptions {
  ...
  readonly createContext?: () => BaseAudioContext; // injecté pour test / offline
  readonly timer?: TimerSource; // worker par défaut, fake en test
}
```

**Plan d'automation (interne, clé de la testabilité)** : `bass-plan.ts` ne
touche pas Web Audio. Il prend `(step, nextStep, params, tStart, stepDuration)`
et retourne une liste de `ParamEvent` du type
`{ target: 'frequency' | 'cutoff' | 'q' | 'vca', kind: 'set' | 'expRamp' | 'target', value, time, tau? }`.
`bass-voice.ts` applique ces événements aux `AudioParam`. Les sémantiques
accent / slide / liaison sont donc testables par assertions sur des données.
...
- `mapping.ts` : courbes cutoff (exponentielle 20 Hz–~8 kHz), résonance (Q borné), decay (temps exponentiel), env mod, drive.
...
- Accent dans `bass-plan.ts` : pic de VCA, ouverture de filtre supplémentaire, résonance poussée ; profondeur par le knob accent.
- Slide : glissé exponentiel de fréquence sur `SLIDE_TIME_S`, **sans** événement d'enveloppe ; note tenue jusqu'au prochain pas joué (liaison à travers les silences) ; gestion du bouclage 15 → 0.
...
| `bass-plan.ts`                                                            | note + octave + tuning → fréquence attendue ; silence → aucun événement de gate ; **slide → événement de rampe de fréquence et aucun événement d'enveloppe** ; liaison à travers un silence → le VCA reste ouvert ; accent → valeurs de pic supérieures sur VCA, cutoff et Q ; toutes les valeurs de rampes exponentielles > 0 ; les temps sont strictement croissants         |
...
| `mapping.ts`                                                              | bornes (0 → min, 1 → max), monotonie, cutoff exponentiel                                                                                                                                                                                                                                                                                                                       |
...
### 6.3 Tests de composants (Vitest + jsdom + @testing-library/svelte)
...
| **Rampes exponentielles et zéro** (`exponentialRampToValueAtTime(0)` lève une exception ; une valeur de départ à 0 ne rampe pas)                                                                    | Constantes `MIN_GAIN = 1e-4` (≈ −80 dB) et `MIN_FREQUENCY_HZ`, jamais 0 sur un `AudioParam` rampé exponentiellement. `audio/params.ts` centralise des helpers qui clampent et qui posent toujours un `setValueAtTime` d'ancrage avant une rampe. Les decays utilisent `setTargetAtTime` (asymptote, tolère les cibles positives). Test unitaire : aucun `ParamEvent` exponentiel avec valeur ≤ 0.                                                                                          |
...
| **Automations qui se chevauchent** (tempo élevé, slide + accent, knob tourné pendant une enveloppe)                                                                                                 | Avant chaque pas, `cancelScheduledValues(tStep)` puis ré-ancrage `setValueAtTime` avec la valeur calculée par notre propre modèle (on connaît le plan, donc pas besoin de `cancelAndHoldAtTime`, absent de Firefox). Les knobs écrivent via `setTargetAtTime` avec une petite constante de lissage, ce qui se compose avec les enveloppes.                                                                                                                                                 |
...
| **Clics à l'arrêt** ou au changement de forme d'onde                                                                                                                                                | Stop = rampe de VCA sur `STOP_RELEASE_S` puis annulation des événements futurs ; `osc.type` modifiable à chaud sans recréer le nœud.                                                                                                                                                                                                                                                                                                                                                       |
...
| **Biquad instable** à Q élevé + cutoff automatisé vite                                                                                                                                              | Q borné par `RESONANCE_Q_MAX` dans `mapping.ts`. C'est aussi la motivation du filtre en échelle v2 ; l'interface `FilterStage` est prête dès v1.                                                                                                                                                                                                                                                                                                                                           |
...
2. Démo Pages : premier tap → son ; play → métronome absent, groove complet ; les 7 knobs répondent au drag et au doigt ; curseur calé sur le son (vérifier à 60 et 160 BPM).
