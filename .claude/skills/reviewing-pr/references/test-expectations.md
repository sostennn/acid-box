...
ne peut pas exister : vérifier que `fake-audio-context.ts` grandit avec le graphe. Limites
connues du fake au 2026-09-23 : `ParamCall` n'enregistre pas le `timeConstant` de
`setTargetAtTime`, donc les constantes de temps des enveloppes sont invisibles aux tests
de voix ; `cancelScheduledValues` enregistre `value: NaN`, piège pour toute assertion
